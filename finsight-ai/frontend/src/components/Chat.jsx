...
import { getNifty50, getMarketSentiment, streamChat } from '../api';
...
const initialMessages = [
  {
    ...
    role: 'assistant',
    content:
      "Good morning. I'm FinSight AI, your personal market intelligence terminal. I can analyze stocks, track IPOs, or provide SIP recommendations based on current Indian market conditions. What would you like to explore?",
    timestamp: '09:15 AM',
    steps: null,
  },
];
...
export default function Chat() {
    ...
    async function fetchSidebarData() {
      try {
        ...
        if (niftyRes.status === 'fulfilled' && niftyRes.value) {
          const payload = niftyRes.value.index ?? niftyRes.value;
        ...
        if (sentRes.status === 'fulfilled' && sentRes.value) {
          ...
          if (
            sources.economic_times &&
            Array.isArray(sources.economic_times.articles)
          ) {
            allNews.push(
              ...sources.economic_times.articles.map((article) => ({
                ...article,
                source: 'ET',
              }))
            );
          ...
          if (
            ...
            Array.isArray(sources.moneycontrol.articles)
          ) {
            allNews.push(
              ...sources.moneycontrol.articles.map((article) => ({
                ...article,
                source: 'MC',
              }))
            );
          ...
          setNewsData(filteredNews.slice(0, 4));
        }
      } catch (err) {
        console.error('[Chat] Failed to fetch sidebar data:', err);
      }
    }
  ...
  const handleSend = async (e, customMessage = null) => {
    ...
    const messageText = (
      customMessage !== null ? customMessage : inputValue
    ).trim();
    ...
    try {
      ...
      for await (const event of streamChat(messageText)) {
        if (!event || !event.type) {
          continue;
        }

        if (event.type === 'step' && event.content) {
          stepList.push(String(event.content));
        ...
        if (event.type === 'done') {
          ...
          if (!streamError) {
            if (fullText.trim()) {
              ...
              addAssistantMessage(
                'I could not generate a response from the available data. Please try again.'
              );
            }
          }
          setStreamingText('');
      ...
      if (!completed) {
        if (fullText.trim()) {
          ...
          addAssistantMessage(
            'The server closed the connection before generating a response.'
          );
        }
        setStreamingText('');
        setCurrentSteps([]);
      ...
      console.error('[Chat] Request failed:', networkErr);
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: 'assistant',
          content:
            `Could not connect to the FinSight AI server.\n\n` +
    ...
    <div className="flex flex-col lg:flex-row h-full gap-6 pb-6">
      ...
      <Card className="flex-1 flex flex-col h-full bg-white relative overflow-hidden flex-shrink-0 lg:w-[60%]">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-[#E5E7EB] bg-white z-10">
          ...
          <div className="flex items-center gap-2 bg-[#22C55E]/10 pl-2 pr-3 py-1 rounded-full">
            ...
            <span className="text-[11px] font-semibold text-[#22C55E] uppercase tracking-[0.08em]">
              Live
            </span>
          </div>
        </div>
          ...
          {messages.map((msg) => (
            <div
              ...
              className={`flex flex-col ${msg.role === 'user'
                ? 'items-end'
                : 'items-start'
                } max-w-full animate-in slide-in-from-bottom-2 fade-in duration-300 ease-out`}
            >
              {/* Agentic Steps */}
              {msg.role === 'assistant' &&
                msg.steps &&
              ...
              <span
                className={`text-[11px] text-[#6B7280] mt-1.5 ${msg.role === 'assistant' ? 'ml-10' : ''
                  }`}
              >
                {msg.timestamp}
              </span>
                ...
                <div className="mb-3 w-full ml-10 flex flex-col gap-1.5 transition-all duration-300">
                  {currentSteps.map((step, i) => (
                    ...
                    >
                      <Badge
                        variant="neutral"
                        className="flex items-center gap-1.5 !bg-[#F3F4F6] !text-[#6B7280] !py-1 px-2.5"
                      >
            ...
            <div className="flex flex-col items-start max-w-[80%] animate-in slide-in-from-bottom-2 fade-in duration-300">
              <div className="flex gap-3 w-full">
                ...
                <div className="px-4 py-3 text-[14px] leading-relaxed bg-[#F7F8F5] text-[#111111] rounded-[16px_16px_16px_4px] shadow-sm prose prose-sm max-w-none">
                  ...
                  <span className="inline-block w-1.5 h-4 ml-0.5 align-middle bg-[#111111] animate-blink" />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-2 mb-3 overflow-x-auto pb-1 scrollbar-hide">
            {[
              ...
              <button
                ...
                type="button"
                onClick={() =>
                  handleQuickAction(pill)
                }
                disabled={isTyping}
                className="px-3 py-1.5 text-xs font-medium rounded-full border border-[#E5E7EB] bg-white text-[#111111] hover:bg-[#111111] hover:text-white transition-all duration-200 whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {pill}
              </button>
            ))}
      ...
      <div className="lg:w-[40%] flex flex-col gap-6 overflow-y-auto pr-1 pb-6 hide-scrollbar flex-shrink-0">
        <StockCard
          symbol="NIFTY 50"
          name="Market Index"
          price={marketData?.value ?? null}
          change={marketData?.change_pct ?? null}
          isPositive={
        ...
        <SentimentBar
          score={
            sentimentData && sentimentData.overall_score != null
              ? ((sentimentData.overall_score + 1) / 2) * 100
              : null
          }
        />
